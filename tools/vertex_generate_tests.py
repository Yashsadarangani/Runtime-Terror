...
def get_java_files_list(source_dir):
    """Get list of all Java files in the source directory."""
    java_files = []
        ...
        for file in files:
            if file.endswith(".java"):
                ...
                if not should_skip_file(file_path):
                    java_files.append((root, file, file_path))
    return java_files
...
if __name__ == "__main__":
    ...
    java_files = get_java_files_list(args.source_dir)
    total_files = len(java_files)
    
    print(f"📋 Found {total_files} Java files to process")
    print(f"🎯 Target: Generate {args.max_tests} test files")
    
    # Process files until we reach the maximum number of successful generations
    for i, (root, file, file_path) in enumerate(java_files):
        # Stop if we've reached the maximum number of successful generations
        if successful_generations >= args.max_tests:
            print(f"🎉 Reached target of {args.max_tests} test files!")
            break
            
        print(f"\n📝 Processing file {i+1}/{total_files}: {file}")
        ...
        try:
            ...
            print(f"📊 Progress: {successful_generations}/{args.max_tests} tests generated")
            
            if generate_tests(token, project, code, class_name, package_name, args.out_dir, relative_path):
                successful_generations += 1
                print(f"✅ Success! Generated test {successful_generations}/{args.max_tests}")
            else:
    ...
    print(f"\n" + "="*50)
    print(f"📊 Final Test Generation Summary:")
    print(f"✅ Successful: {successful_generations}/{args.max_tests}")
    print(f"❌ Failed: {failed_generations}")
    print(f"📁 Output directory: {args.out_dir}")
    print(f"⏱️  Total files processed: {successful_generations + failed_generations}")
    print(f"📈 Success rate: {(successful_generations/(successful_generations + failed_generations)*100):.1f}%" if (successful_generations + failed_generations) > 0 else "N/A")
    
    if successful_generations < args.max_tests:
        print(f"\n⚠️  Warning: Only generated {successful_generations} out of {args.max_tests} requested tests")
        if failed_generations > 0:
            print(f"💡 Consider running again to retry failed generations")
    
    # Exit with success if we generated at least some tests
